  ...
  return (
    <div>
      <header style={{ marginBottom: '60px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button 
          onClick={() => setModal({ isNew: true, name: '', description: '', imageUrl: '' })}
          ...
          <div key={p.id} style={{ backgroundColor: '#fff', border: '1px solid #eee', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '25px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setModal({ isNew: false, id: p.id, name: p.name, description: p.description, imageUrl: p.imageUrl, variantCount: p.variantCount })} style={{ flex: 1, padding: '10px', backgroundColor: '#f5f5f5', border: 'none', borderRadius: '6px', fontSize: '0.8rem', cursor: 'pointer' }}>EDIT</button>
              </div>
            </div>
          </div>
        ))}
